...
static INLINE real_t MUL_C(real_t x, real_t y)
{
    real_t ret;
...
static INLINE real_t MUL_F(real_t x, real_t y)
{
    real_t ret;
...
static INLINE real_t MUL_Q2(real_t x, real_t y)
{
    real_t ret;
...
static INLINE real_t MUL_SHIFT6(real_t x, real_t y)
{
    real_t ret;
...
static INLINE real_t MUL_SHIFT23(real_t x, real_t y)
{
    real_t ret;
...
#else
/* without shift
 * the result may be diffrent from MACRO define
 * but improve the speed too much
 * */

/* taken from MAD */
#define csky_mul(x, y, SCALEBITS) \
({ \
    real_t ret; \
    asm("mul.s32         l0, %1, %2\n\t" \
        "dexti           %0, l0, l1, %3\n\t" \
...
static INLINE real_t MUL_R(real_t A, real_t B)
{
    return csky_mul(A, B, REAL_BITS);
}

static INLINE real_t MUL_C(real_t A, real_t B)
{
    return csky_mul(A, B, COEF_BITS);
}

static INLINE real_t MUL_Q2(real_t A, real_t B)
{
    return csky_mul(A, B, Q2_BITS);
}

static INLINE real_t MUL_SHIFT6(real_t A, real_t B)
{
    return csky_mul(A, B, 6);
}

static INLINE real_t MUL_SHIFT23(real_t A, real_t B)
{
    return csky_mul(A, B, 23);
}
