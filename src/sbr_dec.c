                               ...
                               const uint8_t just_seeked, const uint8_t downSampledSBR)
{
    ...
    ALIGN qmf_t X_right[38][64]; /* must set this to 0 */
...
#else
        sbr_qmf_synthesis_64_asm(sbr, sbr->qmfs[0], X_left, left_channel);
        sbr_qmf_synthesis_64_asm(sbr, sbr->qmfs[1], X_right, right_channel);
#endif
    }
